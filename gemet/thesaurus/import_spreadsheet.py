...
def row_dicts(sheet):
    ...
    mandatory_columns = {"Term", "Definition", "Definition source"}
    optional_columns = {
        "Broader concept",
        "Broader URI",
        "Group",
    ...
    column_names = [c.value.strip() for c in next(rows) if c.value and c.value.strip()]
...
class Importer(object):
    ...
    def import_file(self):
        ...
        num_reg_concepts_bef = Concept.objects.filter(namespace=self.concept_ns).count()
        ...
        if translation_sheetnames:
            ...
            ).format(len(translation_sheetnames), ", ".join(translation_sheetnames))
    ...
    def _create_concepts(self, sheet):
        for i, row in enumerate(row_dicts(sheet)):
            ...
            alt_labels = [
                row[key] for key in row.keys() if "Alt Label" in key and row[key]
            ]
    ...
    def _create_row_relations(self, i, row):
        ...
        for property_type in self.property_types:
            ...
            if property_type.name == "broader":
                ...
                target_labels = [
                    row[key] for key in row.keys() if "Group" in key and row[key]
                ]
            elif property_type.name == "theme":
                target_labels = [
                    row[key] for key in row.keys() if "Theme" in key and row[key]
                ]
            ...
            for target_label in target_labels:
                target = (
                    ...
                    .exclude(status=DELETED_PENDING, properties__status=DELETED_PENDING)
                    .first()
                )

                if not target:
                    raise ImportError(
                        'Row {}: concept "{}" does not exist.'.format(i, target_label)
                    )
                ...
                relation, created = Relation.objects.get_or_create(
                    ...
                    defaults={"version_added": self.version, "status": PENDING},
                )
                ...
                if not relation.reverse:
                    ...
                    print("Reverse relation created: {}".format(reverse_relation))
        if has_broader:
            created = source.inherit_groups_and_themes_from_broader()
            print(
                "Inherited groups and themes from: {}".format(", ".join(broader_labels))
            )
