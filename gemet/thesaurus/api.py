...
def getTopmostConcepts(thesaurus_uri, language=DEFAULT_LANGCODE):
    ...
    excluded_concepts = (
        Relation.published.filter(
            property_type__name="narrower", target__namespace_id=ns.id
        )
...
def getAllConceptRelatives(concept_uri, target_thesaurus_uri=None, relation_uri=None):
        ...
        relatives.append(
            {
                ...
                "target": relation["target__namespace__url"] + relation["target__code"],
            }
        )
    return relatives
...
def hasRelation(concept_uri, relation_uri, object_uri):
    ...
    property_type_uri = (
        ...
        .values_list("property_type__uri", flat=True)
        .first()
    )
...
def getConceptsMatchingKeyword(
    keyword, search_mode, thesaurus_uri="", language=DEFAULT_LANGCODE
):
    try:
...
def getConceptsMatchingRegexByThesaurus(
    regex, thesaurus_uri, language=DEFAULT_LANGCODE
):
    language = get_language(language)
    ns = get_namespace(thesaurus_uri)
...
ApiView.register_function(getAllTranslationsForConcept, "getAllTranslationsForConcept")
ApiView.register_function(getConceptsMatchingKeyword, "getConceptsMatchingKeyword")
ApiView.register_function(
    getConceptsMatchingRegexByThesaurus, "getConceptsMatchingRegexByThesaurus"
)
