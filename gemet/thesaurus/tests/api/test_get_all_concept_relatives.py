...
class TestAllConceptRelatives(GemetTest):
    ...
    def test_bad_relation(self):
        ...
        p1 = PropertyTypeFactory(
            id=1, name="related", label="related", uri="in_relation"
        )
        p2 = PropertyTypeFactory(
            id=2, name="related", label="related", uri="in_relation"
        )
    ...
    def test_all_3_parameters(self):
        term2 = TermFactory(id=2, code="2")
        p1 = PropertyTypeFactory(
            id=1, name="broader", label="broader term", uri="broader_uri"
        )
        p2 = PropertyTypeFactory(
            id=2, name="narrower", label="narrower term", uri="narrower_uri"
        )
        RelationFactory(property_type=p1, source=self.term, target=term2)
        RelationFactory(property_type=p2, source=term2, target=self.term)

        theme = ThemeFactory()
        p3 = PropertyTypeFactory(
            id=3, name="broader", label="broader term", uri="broader_uri"
        )
        p4 = PropertyTypeFactory(
            id=4, name="narrower", label="narrower term", uri="narrower_uri"
        )
        ...
        p5 = PropertyTypeFactory(
            id=5, name="related", label="related", uri="related_uri"
        )
        p6 = PropertyTypeFactory(
            id=6, name="related", label="related", uri="related_uri"
        )
        ...
        resp = resp.pop()
        self.assertEqual(resp["source"], self.NS_ROOT + "concept/" + self.term.code)
        self.assertEqual(resp["relation"], p3.uri)
        self.assertEqual(resp["target"], self.NS_ROOT + theme.get_about_url()[1:])
    ...
    def test_optional_parameters(self):
        ...
        theme = ThemeFactory()
        p3 = PropertyTypeFactory(id=3, name="theme", label="Theme", uri="theme_uri")
        p4 = PropertyTypeFactory(
            id=4, name="themeMember", label="Theme member", uri="theme_member_uri"
        )
        RelationFactory(property_type=p3, source=self.term, target=theme)
        RelationFactory(property_type=p4, source=theme, target=self.term)
        ...
        self.assertEqual(resp[0]["source"], self.NS_ROOT + "concept/" + self.term.code)
        self.assertEqual(resp[0]["relation"], p1.uri)
        self.assertEqual(resp[0]["target"], self.NS_ROOT + term2.get_about_url()[1:])
        self.assertEqual(resp[1]["source"], self.NS_ROOT + "concept/" + self.term.code)
        self.assertEqual(resp[1]["relation"], p3.uri)
        self.assertEqual(resp[1]["target"], self.NS_ROOT + theme.get_about_url()[1:])
