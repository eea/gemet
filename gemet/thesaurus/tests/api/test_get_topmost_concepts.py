...
class TestGetTopmostConcepts(GemetTest):
    ...
    def test_default_language_parameter(self):
        ...
        self.assertEqual(
            resp.json.pop()["preferredLabel"]["language"], DEFAULT_LANGCODE
        )
    ...
    def test_invalid_language(self):
        self.assertRaises(
            ...
            self.app.get,
            self.url
            + urlencode({"thesaurus_uri": self.term.namespace.url, "language": "es"}),
        )
    ...
    def test_invalid_thesaurus_uri(self):
        self.assertRaises(
            ...
            self.url
            + urlencode(
                {"thesaurus_uri": "BAD_THESAURUS_URI", "language": self.english.code}
            ),
        )

    def test_missing_thesaurus_uri(self):
        self.assertRaises(
            Fault, self.app.get, self.url + urlencode({"language": self.english.code})
        )
    ...
    def test_one_term(self):
        ...
        self.assertEqual(resp["uri"], self.NS_ROOT + self.term.get_about_url()[1:])
    ...
    def test_two_terms_without_broader_narrower(self):
        ...
        self.assertEqual(resp[0]["uri"], self.NS_ROOT + self.term.get_about_url()[1:])
        ...
        self.assertEqual(resp[1]["uri"], self.NS_ROOT + term2.get_about_url()[1:])
    ...
    def test_two_languages(self):
        ...
        resp = self.app.get(
            self.url
            + urlencode(
                {"thesaurus_uri": self.term.namespace.url, "language": spanish.code}
            )
        )
        ...
        self.assertEqual(resp["uri"], self.NS_ROOT + term2.get_about_url()[1:])
    ...
    def test_two_namespaces(self):
        ...
        self.assertEqual(resp["definition"]["string"], "definition2")
        self.assertEqual(resp["definition"]["language"], "en")
        self.assertEqual(resp["uri"], self.NS_ROOT + theme.get_about_url()[1:])
