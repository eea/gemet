...
class TestSearchView(GemetTest):
    def setUp(self):
        ...
        PropertyFactory(
            concept=cp1,
            value="{0}something{0}{0}not{0}hiddenLabel{0}".format(SEARCH_SEPARATOR),
            name="searchText",
        )
        PropertyFactory(
            concept=cp2,
            value="{0}something else{0}altLabel{0}{0}{0}".format(SEARCH_SEPARATOR),
            name="searchText",
        )
        PropertyFactory(
            concept=cp3,
    ...
    def test_no_results(self):
        ...
        self.assertEqual(
            resp.pyquery(".content span").text(),
    ...
    def test_multiple_results(self):
        ...
        self.assertEqual(
            resp.pyquery(".content ul li:eq(0) a").text(), "something else"
        )
        self.assertEqual(resp.pyquery(".content ul li:eq(1) a").text(), "something")
    ...
    def test_number_of_results_found(self):
        ...
        self.assertEqual(resp.pyquery(".content span")[0].text, "Found 2 results for '")
    ...
    def test_regex_search(self):
        ...
        self.assertEqual(
            resp.pyquery(".content li:eq(0) a").text(), "another somefling"
        )
        self.assertEqual(resp.pyquery(".content li:eq(1) a").text(), "something else")
        self.assertEqual(resp.pyquery(".content li:eq(2) a").text(), "something")
    ...
    def test_other_names(self):
        ...
        self.assertEqual(len(resp.pyquery(".content li")), 2)
        self.assertEqual(
            resp.pyquery(".content ul > li:eq(0) .py-other-names").text(), "altLabel"
        )
        self.assertEqual(
