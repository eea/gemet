...
class TestSuperGroupView(GemetTest):
    def setUp(self):
        ...
        PropertyFactory(
            concept=self.supergroup, name="prefLabel", value="some prefLabel"
        )

    def test_supergroup_no_concept(self):
        url = reverse(
            "supergroup", kwargs={"code": self.supergroup.code, "langcode": "en"}
        )
        ...
        self.assertEqual(
            resp.pyquery(".content ul li").text(), "English some prefLabel"
        )

    def test_supergroup_one_group(self):
        group = GroupFactory()
        PropertyFactory(concept=group, name="prefLabel", value="group prefLabel")
        ...
        self.assertEqual(resp.pyquery(".content ul.listing").children().size(), 1)
        self.assertEqual(
            resp.pyquery(".content ul.listing li").text(), "group prefLabel"
        )
        self.assertEqual(resp.pyquery(".content ul#translations").children().size(), 1)
        self.assertEqual(
            resp.pyquery(".content ul#translations li").text(), "English some prefLabel"
        )

    def test_supergroup_two_groups(self):
        group1 = GroupFactory(code="1")
        PropertyFactory(concept=group1, name="prefLabel", value="group1 prefLabel")
        pt1 = PropertyTypeFactory(name="narrower", label="narrower term")
        pt2 = PropertyTypeFactory(name="broader", label="broader term")
        RelationFactory(property_type=pt1, source=self.supergroup, target=group1)
        RelationFactory(property_type=pt2, source=group1, target=self.supergroup)
        group2 = GroupFactory(code="2")
        PropertyFactory(concept=group2, name="prefLabel", value="group2 prefLabel")
        pt3 = PropertyTypeFactory(name="narrower", label="narrower term")
        pt4 = PropertyTypeFactory(name="broader", label="broader term")
        RelationFactory(property_type=pt3, source=self.supergroup, target=group2)
        RelationFactory(property_type=pt4, source=group2, target=self.supergroup)
        url = reverse(
            "supergroup", kwargs={"code": self.supergroup.code, "langcode": "en"}
        )
        resp = self.app.get(url)
        ...
        self.assertEqual(resp.pyquery(".content ul").size(), 2)
        self.assertEqual(resp.pyquery(".content ul.listing").children().size(), 2)
        self.assertEqual(
            resp.pyquery(".content ul.listing li:eq(0)").text(), "group1 prefLabel"
        )
        self.assertEqual(
            resp.pyquery(".content ul.listing li:eq(1)").text(), "group2 prefLabel"
        )
        self.assertEqual(resp.pyquery(".content ul#translations").children().size(), 1)
        self.assertEqual(
...
class TestSuperGroupViewWithUser(GemetTest):
    ...
    def test_relations_pending(self):
        ...
        RelationFactory(
            property_type=pt1, source=self.supergroup, target=self.group, status=PENDING
        )
        RelationFactory(
    ...
    def test_relations_prefLabel_pending(self):
        self.group = GroupFactory()
        PropertyFactory(concept=self.group, value="OldGroup", status=DELETED_PENDING)
