...
class TestGroupsView(GemetTest):
    ...
    def setUp(self):
        ...
        self.supergroup_name = PropertyFactory(
            concept=self.supergroup, value="Super Group"
        )
    ...
    def test_one_supergroup_one_group(self):
        ...
        self.assertEqual(
            resp.pyquery(".content .listing.no-list h3").text(), "Super Group"
        )
        self.assertEqual(resp.pyquery(".content .groups li").size(), 1)
        self.assertEqual(
            resp.pyquery(".content .groups li a").attr("href"),
            reverse("relations", kwargs={"langcode": "en", "group_code": group.code}),
        )
    ...
    def test_more_supergroups_no_group(self):
        ...
        self.assertEqual(200, resp.status_int)
        self.assertEqual(resp.context["language"].code, "en")
        self.assertEqual(resp.pyquery(".content .listing.no-list > li").size(), 2)
        self.assertEqual(resp.pyquery(".content .groups li").size(), 0)
        self.assertEqual(
            resp.pyquery(".content .listing.no-list li:eq(0) h3").text(), "Super Group"
        )
        self.assertEqual(
    ...
    def test_more_supergroups_one_group(self):
        ...
        self.assertEqual(resp.pyquery(".content .groups li:eq(0)").text(), "Group")
        self.assertEqual(
...
class TestGroupsViewWithUser(GemetTest):
    def setUp(self):
        self.supergroup = SuperGroupFactory()
        self.supergroup_name = PropertyFactory(
            concept=self.supergroup, value="Super Group", status=PUBLISHED
        )
        user = UserFactory()
        LanguageFactory()
    ...
    def test_supergroup_name_pending(self):
        ...
        PropertyFactory(
            concept=self.supergroup, value="Super Group New", status=PENDING
        )
        url = reverse("groups", kwargs={"langcode": "en"})
    ...
    def test_supergroup_group_relation_pending(self):
        group = GroupFactory()
        PropertyFactory(concept=group, value="Group", status=PUBLISHED)

        pt1 = PropertyTypeFactory(name="narrower", label="narrower term")
        pt2 = PropertyTypeFactory(name="broader", label="broader term")
        RelationFactory(
            property_type=pt1, source=self.supergroup, target=group, status=PENDING
        )
        RelationFactory(
            property_type=pt2, source=group, target=self.supergroup, status=PENDING
        )
        url = reverse("groups", kwargs={"langcode": "en"})
        resp = self.app.get(url, user=self.user)
        self.assertEqual(resp.pyquery(".content .listing.no-list > li").size(), 1)
    ...
    def test_new_supergroup(self):
        ...
        PropertyFactory(concept=supergroup, value="New Super Group", status=PENDING)
        url = reverse("groups", kwargs={"langcode": "en"})
        resp = self.app.get(url, user=self.user)
        self.assertEqual(
    ...
    def test_group_name_pending(self):
        group = GroupFactory()
        PropertyFactory(concept=group, value="Group", status=PENDING)
