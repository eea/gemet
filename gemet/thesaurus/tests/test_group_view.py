...
class TestGroupView(GemetTest):
    def setUp(self):
        ...
        PropertyFactory(concept=self.group, name="prefLabel", value="some prefLabel")

    def test_group_no_concept(self):
        url = reverse("group", kwargs={"code": self.group.code, "langcode": "en"})
        ...
        self.assertEqual(
            resp.pyquery(".content p.alert:eq(0)").text(),
    ...
    def test_group_one_concept_one_supergroup(self):
        ...
        PropertyFactory(
            concept=supergroup, name="prefLabel", value="supergroup prefLabel"
        )
        ...
        self.assertEqual(resp.pyquery(".listing:eq(1)").text(), "concept prefLabel")
        self.assertEqual(resp.pyquery(".listing:eq(0)").size(), 1)
        self.assertEqual(resp.pyquery(".listing:eq(0)").text(), "supergroup prefLabel")
...
class TestGroupViewWithUser(GemetTest):
    ...
    def test_relations_pending(self):
        ...
        RelationFactory(
            property_type=pt1, source=self.concept, target=self.group, status=PENDING
        )
        RelationFactory(
            property_type=pt2, source=self.group, target=self.concept, status=PENDING
        )
        RelationFactory(
            property_type=pt3, source=self.group, target=self.supergroup, status=PENDING
        )
        RelationFactory(
    ...
    def test_relations_prefLabel_pending(self):
        self.supergroup = SuperGroupFactory()
        PropertyFactory(
            concept=self.supergroup, value="SuperGroupOld", status=DELETED_PENDING
        )
        PropertyFactory(concept=self.supergroup, value="SuperGroupNew", status=PENDING)
        pt1 = PropertyTypeFactory(name="broader", label="Broader")
        pt2 = PropertyTypeFactory(name="narrower", label="Narrower")
        RelationFactory(
            property_type=pt1, source=self.group, target=self.supergroup, status=PENDING
        )
        RelationFactory(
            property_type=pt2, source=self.supergroup, target=self.group, status=PENDING
        )
