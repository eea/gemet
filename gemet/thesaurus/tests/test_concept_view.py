...
class TestConceptView(GemetTest):
    def setUp(self):
        ...
        PropertyFactory(
            concept=self.concept, name="definition", value="some definition"
        )
    ...
    def set_concept_one_theme(self):
        ...
        RelationFactory(property_type=pt2, source=self.concept, target=self.group)
        RelationFactory(property_type=pt1, source=self.group, target=self.concept)

        pt3 = PropertyTypeFactory(name="themeMember", label="Theme member")
        pt4 = PropertyTypeFactory(name="theme", label="Theme")
        RelationFactory(property_type=pt4, source=self.concept, target=self.theme)
        RelationFactory(property_type=pt3, source=self.theme, target=self.concept)
    ...
    def set_concept_two_theme(self):
        ...
        pt3 = PropertyTypeFactory(name="themeMember", label="Theme member")
        pt4 = PropertyTypeFactory(name="theme", label="Theme")
        RelationFactory(property_type=pt4, source=self.concept, target=self.theme1)
        RelationFactory(property_type=pt3, source=self.theme1, target=self.concept)
        RelationFactory(property_type=pt4, source=self.concept, target=self.theme2)
        RelationFactory(property_type=pt3, source=self.theme2, target=self.concept)

    def test_concept_one_theme(self):
        self.set_concept_one_theme()
        url = reverse("concept", kwargs={"code": self.concept.code, "langcode": "en"})
        ...
        self.assertEqual(
            resp.pyquery(".content #definition p").text(), "some definition"
        )
        self.assertEqual(resp.pyquery(".content #scopeNote").text(), "some scope note")
        self.assertEqual(resp.pyquery("ul.listing:eq(1) li").text(), "Group Parent")
        self.assertEqual(resp.pyquery("ul.listing:eq(0) li").text(), "Theme Parent")

    def test_concept_one_theme_rdf(self):
        self.set_concept_one_theme()
        url = reverse("concept", kwargs={"code": self.concept.code, "langcode": "en"})
        resp = self.app.get(url, extra_environ={"HTTP_ACCEPT": "application/rdf+xml"})

        self.assertEqual(200, resp.status_int)
        self.assertEqual(resp.headers["Content-Type"], "application/rdf+xml")
        self.assertEqual(
            resp.body.count(("concept/" + self.concept.code).encode("ascii")), 1
        )
        self.assertEqual(
            resp.body.count(("group/" + self.group.code).encode("ascii")), 1
        )
        self.assertEqual(
            resp.body.count(("theme/" + self.theme.code).encode("ascii")), 1
        )
    ...
    def test_concept_two_themes(self):
        ...
        self.assertEqual(resp.pyquery(".content #scopeNote").text(), "some scope note")
        self.assertEqual(resp.pyquery("ul.listing:eq(1)").text(), "Group Parent")
    ...
    def test_concept_two_themes_rdf(self):
        ...
        self.assertEqual(
            resp.body.count(("theme/" + self.theme1.code).encode("ascii")), 1
        )
        self.assertEqual(
            resp.body.count(("theme/" + self.theme2.code).encode("ascii")), 1
        )
    ...
    def test_redirect_rdf(self):
        url = reverse(
            "concept_redirect",
            kwargs={"concept_type": "concept", "concept_code": self.concept.code},
        )
        resp = self.app.get(url, extra_environ={"HTTP_ACCEPT": "application/rdf+xml"})
        self.assertEqual(200, resp.status_int)
        self.assertEqual(resp.headers["Content-Type"], "application/rdf+xml")
    ...
    def test_redirect_no_concept_type(self):
        url = reverse(
            "concept_redirect",
            kwargs={"concept_type": "unknown", "concept_code": self.concept.code},
        )
...
class TestConceptViewWithUser(GemetTest):
    ...
    def test_properties(self):
        ...
        self.assertEqual(
            resp.pyquery(".content #definition p").text(), "definition new"
        )
    ...
    def test_user_sees_default_name(self):
        ...
        url = reverse("concept", kwargs={"langcode": "ro", "code": concept4.code})
        resp = self.app.get(url)
        self.assertEqual(resp.pyquery("title").text(), "Concept4 [english]")
        self.assertEqual(resp.pyquery("#prefLabel").text(), "Concept4 [english]")
