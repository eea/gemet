...
class TestExports(GemetTest):
    ...
    def test_themes_groups_relations_html(self):
        ...
        for i in range(0, 4):
            self.assertTrue(resp.pyquery("tbody tr:eq(%s)" % (str(i))).text() in rows)
    ...
    def test_label_and_definitions_html(self):
        ...
        PropertyFactory(
            concept=spanish_term, language=spanish, name="prefLabel", value="X"
        )
    ...
    def test_gemet_groups_html(self):
        ...
        self.assertEqual(resp.pyquery("table:eq(0) tbody td:eq(0)").text(), "10")
        self.assertEqual(
            resp.pyquery("table:eq(0) tbody td:eq(1)").text(), "SuperGroup"
        )
        self.assertEqual(resp.pyquery("table:eq(0) tbody td:eq(2)").text(), "")
        self.assertEqual(resp.pyquery("table:eq(0) tbody td:eq(3)").text(), "A")
        self.assertEqual(resp.pyquery("table:eq(1) tbody tr").size(), 1)
        self.assertEqual(resp.pyquery("table:eq(1) tbody td:eq(0)").text(), "11")
        self.assertEqual(resp.pyquery("table:eq(1) tbody td:eq(1)").text(), "Group")
        self.assertEqual(resp.pyquery("table:eq(1) tbody td:eq(2)").text(), "10")
        self.assertEqual(resp.pyquery("table:eq(1) tbody td:eq(3)").text(), "B")
        self.assertEqual(resp.pyquery("table:eq(2) tbody tr").size(), 1)
        self.assertEqual(resp.pyquery("table:eq(2) tbody td:eq(0)").text(), "12")
        self.assertEqual(resp.pyquery("table:eq(2) tbody td:eq(1)").text(), "Theme")
        self.assertEqual(resp.pyquery("table:eq(2) tbody td:eq(2)").text(), "C")
    ...
    def test_gemet_relations_html(self):
        ...
        rows = [
            "11 Narrower 12",
            "11 Exactmatch concept_uri1",
            "11 Closematch concept_uri2",
            "12 Broader 11",
            "12 Related 13",
            "12 Closematch concept_uri3",
            "13 Related 12",
        ]
    ...
    def test_label_and_definitions_rdf(self):
        ...
        PropertyFactory(concept=term2, name="definition", value="B_definition")

        spanish = LanguageFactory(code="es", name="Spanish")
        term3 = TermFactory(code="13")
        PropertyFactory(
            concept=term3, language=spanish, name="prefLabel", value="C_label"
        )
        PropertyFactory(
            concept=term3, language=spanish, name="definition", value="C_definition"
        )
    ...
    def test_gemet_groups_rdf(self):
        ...
        PropertyFactory(
            concept=es_supergroup,
            language=spanish,
            name="prefLabel",
            value="A_label_spanish",
        )
        PropertyFactory(
            concept=es_group,
            language=spanish,
