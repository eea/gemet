...
class TestThemesView(GemetTest):
    ...
    def test_one_theme(self):
        ...
        self.assertEqual(
            resp.pyquery(".listing.columns li a").attr("href"),
            reverse(
                "theme_concepts", kwargs={"langcode": "en", "theme_code": theme.code}
            ),
        )
        self.assertEqual(
            resp.pyquery(".listing.columns li a").text(), u"administration"
        )
    ...
    def test_contains_more_themes(self):
        ...
        self.assertEqual(
            ...
            reverse(
                "theme_concepts", kwargs={"langcode": "en", "theme_code": theme1.code}
            ),
        )
        self.assertEqual(resp.pyquery(".listing.columns li:eq(0) a").text(), u"Theme 1")
        self.assertEqual(
            resp.pyquery(".listing.columns li:eq(1) a").attr("href"),
            reverse(
                "theme_concepts", kwargs={"langcode": "en", "theme_code": theme2.code}
            ),
        )
        self.assertEqual(resp.pyquery(".listing.columns li:eq(1) a").text(), u"Theme 2")
...
class TestThemesViewWithUser(GemetTest):
    ...
    def test_pending_name_for_theme(self):
        ...
        PropertyFactory(
            concept=theme,
            value="Old Theme test",
            language=self.language,
            status=DELETED_PENDING,
        )
        PropertyFactory(
            concept=theme,
            value="New Theme test",
            language=self.language,
            status=PENDING,
        )
        url = reverse("themes", kwargs={"langcode": "en"})
        resp = self.app.get(url, user=self.user)
        self.assertEqual(resp.pyquery(".listing.columns li a").text(), "New Theme test")

    def test_new_theme(self):
        theme = ThemeFactory(status=PENDING)
        PropertyFactory(
            concept=theme, value="New", status=PENDING, language=self.language
        )
        url = reverse("themes", kwargs={"langcode": "en"})
        resp = self.app.get(url, user=self.user)
