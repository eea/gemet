...
class UnrelatedConcepts(LoginRequiredMixin, JsonResponseMixin, View):
    def _set_reverse_urls(self, concepts, langcode, relation):
        for concept in concepts:
            concept["name"] = get_concept_name(
                concept["name"], concept["id"], (PENDING, PUBLISHED)
            )
            url_kwargs = {
...
class EditPropertyView(LoginRequiredMixin, JsonResponseMixin, VersionMixin, View):
    def post(self, request, langcode, id, name):
        ...
        field = models.Property.objects.filter(
            language=langcode, concept__id=id, name=name
        )
...
class AddRelationView(LoginRequiredMixin, JsonResponseMixin, VersionMixin, View):
    def post(self, request, source_id, target_id, relation_type):
        ...
        relation = models.Relation.objects.filter(
            source=source, target=target, property_type__name=relation_type
...
class AddPropertyView(LoginRequiredMixin, JsonResponseMixin, VersionMixin, View):
    def post(self, request, langcode, id, name):
        try:
            language = models.Language.objects.get(code=langcode)
            concept = models.Concept.objects.get(id=id)
...
class AddForeignRelationView(LoginRequiredMixin, JsonResponseMixin, VersionMixin, View):
...
class AddConceptView(LoginRequiredMixin, HeaderMixin, VersionMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        new_concept = models.Concept(
            version_added=self.pending_version, namespace=namespace, status=PENDING
        )
        ...
        url = reverse(
            url_name, kwargs={"langcode": self.langcode, "code": new_concept.code}
        )
        return redirect(url)
...
class ReleaseVersionView(LoginRequiredMixin, HeaderMixin, VersionMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        models.Property.objects.filter(
            name="searchText", status=DELETED_PENDING
        ).delete()
...
class ChangeLogView(LoginRequiredMixin, HeaderMixin, VersionMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        published_concepts = (
            set(
                published_concepts.filter(
                    properties__status__in=[PENDING, DELETED_PENDING]
                )
            )
            | set(
                published_concepts.filter(
...
class ConceptChangesView(LoginRequiredMixin, View):
    def get(self, request, langcode, id):
        ...
        concept_details = {}
        properties = concept.properties.filter(
            status__in=[PENDING, DELETED_PENDING], language=language
        ...
        other_language_properties = (
            concept.properties.filter(status__in=[PENDING, DELETED_PENDING])
        ...
        for relation in relations:
            ...
            target_relation = {"target": target_name, "status": relation["status"]}
